...
save_file = os.path.join(config['resultsdir'], 'atlas_activity_results_' + condition + '_' + str(iternum) + '.npz')
...
if not os.path.isfile(save_file):
    ...
    results = timepoint_decoder(data[:, :, np.sum(np.isnan(np.sum(data, axis=0)), axis=0) == 0], windowsize=60, mu=0, nfolds=2)

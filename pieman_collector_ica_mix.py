...
def parse_fname(fname, condition):
    ...
    return int(fname[len('ica' + str(k) + '_mix_results_' + condition + '_'):-4])
...
results_file = os.path.join(config['resultsdir'], 'collated_results_ica_mix' + str(k) + '.pkl')

iterations = []
for c in conditions:
    next_files = lsdir(os.path.join(config['resultsdir'], 'ica' + str(k) + '_mix_results_' + c + '*.npz'))
...
if not os.path.isfile(results_file):
    ...
    for c in conditions:
        for t in iterations:
            next_file = os.path.join(config['resultsdir'], 'ica' + str(k) + '_mix_results_' + c + '_' + str(int(t)) + '.npz')
...
sb.plt.ylim(0, 0.20)
sb.plt.savefig(os.path.join(fig_dir, 'decoding_accuracy_ica_mix' + str(k) + '.pdf'))

...
results_file = os.path.join(config['resultsdir'], 'collated_results_baseline.pkl')
...
if not os.path.isfile(results_file):
    ...
    for c in conditions:
        for t in iterations:
            next_file = os.path.join(config['resultsdir'], 'baseline_results_' + c + '_' + str(int(t)) + '.npz')
...
sb.plt.ylim(0, 0.16)
sb.plt.savefig(os.path.join(fig_dir, 'decoding_accuracy_baseline.pdf'))

...
def parse_fname(fname, condition):
    ...
    return int(fname[len('atlas_ISFC_results_' + condition + '_'):-4])
...
results_file = os.path.join(config['resultsdir'], 'collated_results_atlas_ISFC.pkl')

iterations = []
for c in conditions:
    next_files = lsdir(os.path.join(config['resultsdir'], 'atlas_ISFC_results_' + c + '*.npz'))
...
if not os.path.isfile(results_file):
    ...
    for c in conditions:
        for t in iterations:
            next_file = os.path.join(config['resultsdir'], 'atlas_ISFC_results_' + c + '_' + str(int(t)) + '.npz')
...
sb.plt.ylim(0, 0.16)
sb.plt.savefig(os.path.join(fig_dir, 'decoding_accuracy_atlas_ISFC.pdf'))

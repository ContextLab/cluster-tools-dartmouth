...
results_file = os.path.join(config['resultsdir'], 'collated_results_mix.pkl')

iterations = []
for c in conditions:
    next_files = lsdir(os.path.join(config['resultsdir'], 'mix_results_' + c + '*.npz'))
...
if not os.path.isfile(results_file):
    ...
    for c in conditions:
        for t in iterations:
            next_file = os.path.join(config['resultsdir'], 'mix_results_' + c + '_' + str(int(t)) + '.npz')
